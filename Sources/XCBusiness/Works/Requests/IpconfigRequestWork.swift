...
public extension IpconfigRequestWork {
    static func fire() async throws -> IPConfig {
        let config: IPConfig = try await withThrowingTaskGroup(of: IPConfig.self, returning: IPConfig.self) { group in
            group.addTask {
                let result: [Ip_api_response] = try await XCBusiness.share.run(IpconfigRequestWork("ip_config_1"), returnType: Ip_api_response.self)
                guard let first = result.first else { throw NSError.init(domain: "err", code: -1) }
                return first
            }
            group.addTask {
                let result: [Ip_info_response] = try await XCBusiness.share.run(IpconfigRequestWork("ip_config_2"), returnType: Ip_info_response.self)
                guard let first = result.first else { throw NSError.init(domain: "err", code: -1) }
                return first
            }
